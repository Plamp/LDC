...
public class RemplirListe extends BaseActivity {
	...
	// charge les données de la fenêtre: liste des rayons et liste des produits
	// du rayon
	@Override
					...
					} else// ce n'est pas la liste des produits non plus
					{
						...
						if (jsonMainNode != null) {// si c'est la liste de
							...
							for (int i = 0; i < jsonMainNode.length(); i++) {
								...
								listeDesMapsProduitDsListe
										.add(creerMapProduitAAcheter(idProduit,
												qte, libelle));
							}// fin du for de parcours du json
								// création de l'adapteur pour le remplissage de
								// la liste de course
							SimpleAdapter saListe = new SimpleAdapter(this,
									listeDesMapsProduitDsListe,
									R.layout.produit_a_acheter_layout,
									new String[] { "produitId", "produitLib",
											"listeQte" }, new int[] {
											R.id.itemNumeroProduit,
											R.id.itemLibelleProduit,
											R.id.itemQuantite });
							// on essaye de donner l'adapteur créé à la liste de
							// course
							...
							try {
								...
								Log.i("ListeDeCourse",
										listeDesMapsProduitDsListe.toString());
							}
						} 
