...
public class RemplirListe extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		boutonAjouter.setOnClickListener(new AdapterView.OnClickListener() {
			...
			public void onClick(View v) {
				...
				String noProduit = ((HashMap<String, String>) (spinnerProduit
						.getSelectedItem())).get("produitId");
				String qte = ((EditText) findViewById(R.id.editTextQuantite))
						.getText().toString();
				String adresse = baseUrl
						+ "listeCourse.php?action=ajout&produitId=" + noProduit
						+ "&qte=" + qte;
		...
		spinnerRayon
				.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
					...
					public void onItemSelected(AdapterView<?> parent,
						...
						String nomDuRayon = ((HashMap<String, String>) (spinnerRayon
								.getSelectedItem())).get("rayonLib");
						Log.i("ListeDeCourse", nomDuRayon);
						accessWebService(baseUrl + "listeProduits.php?rayon="
								+ nomDuRayon);
	...
	public List<ModelProduit> listeDesProduit = new ArrayList<ModelProduit>();

	// charge les données de la fenêtre: liste des rayons et liste des
	// produits du rayon
	@Override
	public void traiterDonneesRecues(String jsonResult) {
		{
			try {
				...
				if (jsonMainNode != null) {// si c'est la liste des rayons
					...
					SimpleAdapter sARayon = new SimpleAdapter(this,
							listeDesMapsRayon, R.layout.rayon_layout,
							new String[] { "rayonLib" },
							new int[] { R.id.itemLibelle });
				...
				else // ce n'est pas la liste des rayons
				{
					...
					if (jsonMainNode != null) {// si c'est la liste des produits
						...
						for (int i = 0; i < jsonMainNode.length(); i++) {
							...
							String number = jsonChildNode
									.optString("produitId");
							listeDesMapsProduit.add(creerMapProduit(name,
									number));
						}// fin du for de parcours du json
							// création de l'adapteur pour le choix du rayon
						SimpleAdapter saProduit = new SimpleAdapter(this,
								listeDesMapsProduit, R.layout.produit_layout,
								new String[] { "produitLib" },
								new int[] { R.id.itemLibelleProduit });
						// on essaye de donner l'adapteur créé au spinner
					...
					} else// ce n'est pas la liste des produits non plus
					{
						jsonMainNode = jsonResponse
								.optJSONArray("listeDeCourse");
						Log.i("ListeDeCourse", jsonMainNode.toString());
						if (jsonMainNode != null) {// si c'est la liste de
													// course
							// vidons la map
							listeDesProduit.clear();
							// for de parcours du json
							for (int i = 0; i < jsonMainNode.length(); i++) {
								JSONObject jsonChildNode = jsonMainNode
										.getJSONObject(i);
								String idProduit = jsonChildNode
										.optString("produitId");
								String qte = jsonChildNode
										.optString("listeQte");
								String libelle = jsonChildNode
										.optString("produitLib");
								ModelProduit nouveauProduit = new ModelProduit(
										idProduit, libelle, qte);
								// ajout du nouveau rayon à la liste
								listeDesProduit.add(nouveauProduit);
								// listeDesMapsProduitDsListe.add(creerMapProduitAAcheter(idProduit,
								// qte, libelle));
							}// fin du for de parcours du json
								// création de l'adapteur pour le remplissage
								// de la liste de course
								// SimpleAdapter saListe = new
								// SimpleAdapter(this,listeDesMapsProduitDsListe,R.layout.produit_a_acheter_layout,new
								// String[] {
								// "produitId","produitLib","listeQte" },new
								// int[] {
								// R.id.itemNumeroProduit,R.id.itemLibelleProduit,R.id.itemQuantite});
								// on essaye de donner l'adapteur créé à la
								// liste de course
							ProduitAdapter saListe = new ProduitAdapter(this,
									listeDesProduit);
							try {
								// on associe l'adaptateur à la liste des
								// produits à acheter
								listViewProduits.setAdapter(saListe);
							} catch (NullPointerException e) {

								Log.i("ListeDeCourse",
										listeDesProduit.toString());
							}
